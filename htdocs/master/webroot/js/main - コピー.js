...
$("body").on("click", "#makenewinput", function(e) {
  ...
  //★変更しました
  var newinputvalidationval=$("#MuzFormFieldSettingDetailValidateType").val();
  ...
  var url=urlObj.urlFieldNew;
    ...
    if(date.status=='YES'){
	  ...
	  //★変更しました
	  // formid=>field_id,date_key=>追加,sort_number=>追加
      data = [{ 
        "inputtext" : inputtext,
        ...
        $( ".formgroup").each(function() {        
                  ...
                  group_id = $( ".group_id",this).val();
                        ...
                        $( ".formlistitem",this).each(function() {
                            ...
                            if(field_id_item){
                                field_id.push(field_id_item)
                            } 

                                // hidden_valueを取得
                                // $( ".hidden_value input",this).each(function() {
                                //     var hidden_name=$(this).attr('name');
                                //     var hidden_valie=$(this).val();
                   ...
                   if(field_id.length>0){
                      field_id=field_id.join("_");
                   }else{
                       field_id='';
                   }

                    postdata['data[Form][group_id]'] =group_id;
                    postdata['data[Form][sort_id]'] =field_id;
...
switch (formtype) {
    ...
    case "text":
		  ...
		  //★変更しました
		  //postdata['data[MuzFormFieldSettingDetail][validation]'] =newinputvalidationval;
		  postdata['data[MuzFormFieldSettingDetail][validate_type]'] =newinputvalidationval;
    break;
